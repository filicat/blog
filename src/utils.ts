...
abstract class PostsCollationGroup implements CollationGroup<'notes'> {
  title: string
  url: string
  collations: Collation<'notes'>[]

  constructor(title: string, url: string, collations: Collation<'notes'>[]) {
    this.title = title
    this.url = url
    this.collations = collations
  }

  sortCollationsAlpha(): Collation<'notes'>[] {
    this.collations.sort((a, b) => a.title.localeCompare(b.title))
    return this.collations
  }

  sortCollationsLargest(): Collation<'notes'>[] {
    this.collations.sort((a, b) => b.entries.length - a.entries.length)
    return this.collations
  }

  sortCollationsMostRecent(): Collation<'notes'>[] {
  ...
  add(item: CollectionEntry<'notes'>, collationTitle: string): void {
  ...
  match(rawKey: string): Collation<'notes'> | undefined {
    return this.collations.find((entry) => entry.title === rawKey)
  }

  matchMany(rawKeys: string[]): Collation<'notes'>[] {
...
export class SeriesGroup extends PostsCollationGroup {
  ...
  private constructor(title: string, url: string, items: Collation<'notes'>[]) {
    super(title, url, items)
  }
  // Factory method to create a SeriesGroup instance with async data fetching
  static async build(posts?: CollectionEntry<'notes'>[]): Promise<SeriesGroup> {
